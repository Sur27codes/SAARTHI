...
"""
Aria — Daily Health Calling Agent (SAARTHI)
Flask frontend + Retell AI integration

Pipeline:
  Retell call ends
    → transcript captured
    → Claude AI analyses emotion/intensity/context → 7 clinical scores (JSON)
...
# ─── CORS — Allow web frontend from any origin (configurable for production) ──
...
# ENS_API_URL: use SAARTHI_API_URL for prod (Railway/Render), fallback to local
ENS_API_URL       = os.environ.get("SAARTHI_API_URL") or os.environ.get("ENS_API_URL", "http://localhost:8000")
...
ANALYSIS_PROMPT = """You are ARIA, a precision clinical AI for SAARTHI — an ENS (Empty Nose Syndrome) health monitoring system.

Your mission: perform a deep, nuanced extraction of 7 clinical scores from the call transcript below.
ENS is a debilitating condition and patients often minimise or under-report their symptoms. Compensate for this.

CLINICAL SCORES (output as JSON):
- Air_Sensation:      Air quality/breathing naturalness (1=feels completely natural, 10=completely unnatural/unbearable)
- Nasal_Dryness:      Nasal dryness severity (1=moist/comfortable, 10=cracked/bleeding/deserts dry)
- Nasal_Burning:      Burning/stinging sensation (1=none, 10=constant excruciating burn)
- Suffocation:        Breathlessness/chest tightness/air hunger (1=breathing perfectly, 10=gasping/suffocating)
- Anxiety_Score:      Psychological distress from ENS (1=calm&adjusted, 10=acute panic/despair about breathing)
- Humidity_Level_Pct: Indoor humidity percentage (0–100; default 50 if not mentioned)
- Sleep_Quality_Hrs:  Hours of sleep last night (0–12; default 7 if not mentioned)

SCORING INTENSITY GUIDE:
"perfect/fine/great/normal/no problems"         → 1–2
"tiny bit/barely noticeable/slightly"            → 2–3  
"mild/little/somewhat/occasionally"              → 3–5
"moderate/noticeable/bothering me/quite"         → 5–6
"significant/quite bad/often/a lot"              → 6–8
"severe/terrible/constant/unbearable/can't cope" → 8–10

ENS PATIENT COMMUNICATION PATTERNS (critical — apply these adjustments):
- ENS patients often say "it's okay" while describing severe restriction → raise score by 1–2
- Hedging language ("it's not that bad but...", "I manage but...") → raise scope by 1
- Patients who immediately bring up the topic = higher severity than stated
- Short, clipped answers suggesting difficulty speaking = raise Suffocation by 1–2
- Laughter or deflection when discussing symptoms = often masking distress, raise Anxiety by 1

PROSODY / TONE SIGNALS (infer from word choices):
- Sighing, pausing mid-sentence, trailing off → raise Anxiety_Score +1–2, Suffocation +1
- Fast speech, multiple symptoms mentioned rapid-fire → raise Anxiety_Score +1–2
- Monotone/flat affect when describing serious symptoms → may indicate depression, raise Anxiety +1
- Calm, matter-of-fact, positive framing → reduce Anxiety by 1 (floor at 1)

TEMPORAL CONTEXT:
- "It's been worse lately/this week" → raise all related scores by 1
- "It got better recently" → reduce by 1
- "Today is particularly bad" → raise by 1–2
- "About the same as usual" → keep at baseline

DEFAULT RULES:
- Symptom not mentioned at all → 3 (subclinical mild)
- Everything explicitly said to be fine → 2
- Patient distressed throughout → floor Anxiety_Score at 5

TRANSCRIPT:
{transcript}

Respond with ONLY valid JSON — no markdown, no explanation, no extra text:
{{"Air_Sensation": <1-10>, "Nasal_Dryness": <1-10>, "Nasal_Burning": <1-10>, "Suffocation": <1-10>, "Anxiety_Score": <1-10>, "Humidity_Level_Pct": <0-100>, "Sleep_Quality_Hrs": <0-12>, "reasoning": "<2-3 sentence clinical summary citing specific transcript phrases that drove the scores>"}}"""
